...
def _seed_run(repo: SqlAlchemyRepository, run_id: str = "run_123",
              tenant: str = "tenant_a", idem: str | None = None) -> None:
    repo.ensure_tenant(tenant)
    try:
        repo.create_run(
            run_id=run_id, tenant_id=tenant, created_by="analyst_1",
...
def test_run_idempotent_on_idempotency_key(repo: SqlAlchemyRepository):
    ...
    row = repo.create_run(
        run_id="run_idem_2", tenant_id="tenant_a", created_by="analyst_1",
        objective="other", budget={}, idempotency_key="idem-1")
...
def test_event_append_replay_order(repo: SqlAlchemyRepository):
    _seed_run(repo)
    e1 = repo.append_next_event(run_id="run_123", tenant_id="tenant_a",
                                type=EventType.RUN_STARTED, data={"objective": "x"})
    e2 = repo.append_next_event(run_id="run_123", tenant_id="tenant_a",
                                type=EventType.MESSAGE_DELTA, data={"delta": "hi"})
...
def test_event_append_idempotent_on_event_id(repo: SqlAlchemyRepository):
    _seed_run(repo)
    ev = AgentEvent(event_id="evt_dup", run_id="run_123", tenant_id="tenant_a",
                    sequence=0, type=EventType.RUN_STARTED, timestamp=NOW, data={})
...
def test_event_sequence_conflict(repo: SqlAlchemyRepository):
    _seed_run(repo)
    repo.append_event(AgentEvent(event_id="e1", run_id="run_123", tenant_id="tenant_a",
                                 sequence=0, type=EventType.RUN_STARTED,
                                 timestamp=NOW, data={}))
    with pytest.raises(ConflictError):
        repo.append_event(AgentEvent(event_id="e2", run_id="run_123", tenant_id="tenant_a",
                                     sequence=0, type=EventType.MESSAGE_DELTA,
                                     timestamp=NOW, data={}))
...
def test_event_data_redacted_on_write(repo: SqlAlchemyRepository):
    ...
    ev = repo.append_next_event(
        run_id="run_123", tenant_id="tenant_a", type=EventType.TOOL_STARTED,
...
def test_approval_persist_decide_double_decide(repo: SqlAlchemyRepository):
    ...
    ap = Approval(approval_id="ap_1", run_id="run_123", tenant_id="tenant_a",
                  action_id="act_1", requested_by="orchestrator", requested_at=requested,
                  expires_at=requested + timedelta(minutes=30))
    repo.create_approval(ap)
    decided = repo.decide_approval(approval_id="ap_1", tenant_id="tenant_a",
                                   decision="approved", approver="lead")
    assert decided.decision.value == "approved"
    with pytest.raises(ConflictError):
        repo.decide_approval(approval_id="ap_1", tenant_id="tenant_a",
                             decision="rejected", approver="lead2")


def test_approval_expiry(repo: SqlAlchemyRepository):
    _seed_run(repo)
    base = datetime.now(timezone.utc)
    ap = Approval(approval_id="ap_exp", run_id="run_123", tenant_id="tenant_a",
                  action_id="act_9", requested_by="orchestrator",
                  requested_at=base - timedelta(hours=2),
                  expires_at=base - timedelta(hours=1))
    repo.create_approval(ap)
    with pytest.raises(ConflictError):
        repo.decide_approval(approval_id="ap_exp", tenant_id="tenant_a",
                             decision="approved", approver="lead")
...
def _invocation(iid: str = "inv_1", idem: str | None = None) -> ToolInvocation:
    ...
    return ToolInvocation(
        invocation_id=iid, run_id="run_123", tenant_id="tenant_a",
        tool_name="telemetry.query_metrics", tool_version="1.0.0",
        input_hash="placeholder", redacted_input={},
        status=ToolInvocationStatus.RUNNING, started_at=started,
        idempotency_key=idem)
...
def test_tool_audit_redacts_and_hashes(repo: SqlAlchemyRepository):
    ...
    # Completed with secret-bearing output -> redacted too.
        ...
        invocation_id="inv_1", tenant_id="tenant_a", status="succeeded",
...
def test_tool_idempotency_key_dedupes(repo: SqlAlchemyRepository):
    _seed_run(repo)
    repo.record_tool_invocation(_invocation("inv_a", idem="tool-idem-1"),
                                raw_input={"a": 1})
    dup = repo.record_tool_invocation(_invocation("inv_b", idem="tool-idem-1"),
                                      raw_input={"a": 1})
...
def test_finding_task_modelcall_audit_paths(repo: SqlAlchemyRepository):
    _seed_run(repo)
    repo.save_evidence_ref(ref_id="ev_1", run_id="run_123", tenant_id="tenant_a",
                           kind="metric", uri="telemetry://m/1",
                           excerpt_hash=hash_payload("excerpt"))
    repo.save_finding(finding_id="f_1", run_id="run_123", tenant_id="tenant_a",
                      title="error spike", summary="5xx after release",
                      evidence_refs=["ev_1"], confidence=0.8, severity="high")
    repo.save_agent_task(task_id="task_1", run_id="run_123", tenant_id="tenant_a",
                         skill_id="correlate-service-symptoms",
                         agent_name="observability-agent", objective="correlate",
                         raw_inputs={"service": "checkout-api",
                                     "token": "should-redact"})
    repo.update_agent_task(task_id="task_1", tenant_id="tenant_a",
                           status="completed", raw_output={"cause": "bad release"})
    cid = repo.record_model_call(run_id="run_123", tenant_id="tenant_a",
                                 model="gpt-4o-mini", task_type="fast",
                                 raw_prompt={"messages": "classify this"},
                                 input_tokens=50, output_tokens=10,
                                 cost_usd=0.001, duration_ms=120)
    assert cid.startswith("mcall_")
    aid = repo.write_audit(run_id="run_123", tenant_id="tenant_a",
                           actor="policy", action="policy.decision",
