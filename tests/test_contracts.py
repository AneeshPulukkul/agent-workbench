...
def test_agent_request_rejects_extra_and_bad_budgets():
    ...
    with pytest.raises(ValidationError):
        AgentRequest(
            run_id="run_1", tenant_id="t", user_id="u", objective="o", unknown_field="nope"
        )
...
def test_a2a_request_result_contracts():
    ...
    with pytest.raises(ValidationError):  # completed must not carry error
        A2ATaskResult(task_id="task_1", status="completed", error={"code": "x", "message": "y"})
...
def test_approval_lifecycle():
    ...
    decided = Approval(
        **{**ap.model_dump(), "decision": "approved", "approver": "lead", "decided_at": LATER}
    )
