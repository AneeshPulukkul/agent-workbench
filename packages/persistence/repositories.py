...
def _redact_value(key_path: str, value: Any) -> Any:
    if isinstance(value, dict):
        return {
            k: _redact_value(f"{key_path}.{k}" if key_path else str(k), v)
...
class RunStore(Protocol):
    ...
    def create_run(self, *, run_id: str, tenant_id: str, created_by: str,
                   objective: str, budget: dict[str, Any],
...
class EventStore(Protocol):
    ...
    def append_next_event(self, *, run_id: str, tenant_id: str,
                          type: EventType | str, data: dict[str, Any],
                          event_id: str | None = None,
                          trace_id: str | None = None,
                          correlation_id: str | None = None,
                          sensitive: bool = False,
                          safe_for_ui: bool = False) -> AgentEvent: ...
    def list_events(self, run_id: str, tenant_id: str,
                    after_sequence: int = -1, limit: int = 1000) -> list[AgentEvent]: ...


class ApprovalStore(Protocol):
    def create_approval(self, approval: Approval) -> Approval: ...
    def get_approval(self, approval_id: str, tenant_id: str) -> Approval: ...
    def decide_approval(self, *, approval_id: str, tenant_id: str,
                        decision: str, approver: str,
                        reason: str | None = None) -> Approval: ...


class ToolAuditStore(Protocol):
    def record_tool_invocation(self, invocation: ToolInvocation,
                               raw_input: dict[str, Any] | None = None,
                               raw_output: dict[str, Any] | None = None) -> ToolInvocation: ...
    def complete_tool_invocation(self, *, invocation_id: str, tenant_id: str,
                                 status: str,
                                 raw_output: dict[str, Any] | None = None,
                                 error: dict[str, Any] | None = None) -> ToolInvocation: ...
...
class SqlAlchemyRepository:
    ...
    def create_run(self, *, run_id: str, tenant_id: str, created_by: str,
        ...
        with self.transaction() as s:
            ...
            row = Run(
                id=run_id, tenant_id=tenant_id, created_by=created_by,
                objective=objective, status="created", current_state=current_state,
                budget_json=budget, idempotency_key=idempotency_key,
    ...
    def update_run(self, run_id: str, tenant_id: str, **fields: Any) -> Run:
        allowed = {"status", "current_state", "budget_json", "completed_at",
                   "trace_id", "correlation_id", "objective"}
    ...
    def append_event(self, event: AgentEvent) -> AgentEvent:
        ...
        with self.transaction() as s:
            ...
            dup = s.execute(
                select(RunEvent).where(RunEvent.event_id == event.event_id)
            ).scalars().first()
            if dup is not None:
                return self._row_to_event(dup)
            row = RunEvent(
                event_id=event.event_id, run_id=event.run_id,
                tenant_id=event.tenant_id, sequence=event.sequence,
                type=event.type.value if isinstance(event.type, EventType) else str(event.type),
                timestamp=event.timestamp, data_json=dict(event.data),
                trace_id=event.trace_id, correlation_id=event.correlation_id,
                sensitive=event.sensitive, safe_for_ui=event.safe_for_ui,
            )
            s.add(row)
            try:
                s.flush()
    ...
    def append_next_event(self, *, run_id: str, tenant_id: str,
        ...
        for _ in range(4):  # initial + 3 retries on sequence race only
            with self.transaction() as s:
                ...
                dup = s.execute(
                    select(RunEvent).where(RunEvent.event_id == eid)
                ).scalars().first()
                if dup is not None:
                    return self._row_to_event(dup)
                seq = self._next_sequence_in(s, run_id)
                row = RunEvent(
                    event_id=eid, run_id=run_id, tenant_id=tenant_id,
                    sequence=seq,
                    type=type.value if isinstance(type, EventType) else str(type),
                    timestamp=_now(), data_json=redacted_data,
                    trace_id=trace_id, correlation_id=correlation_id,
                    sensitive=sensitive, safe_for_ui=safe_for_ui,
                )
                s.add(row)
                try:
                    s.flush()
                ...
                return AgentEvent(
                    event_id=eid, run_id=run_id, tenant_id=tenant_id,
                    sequence=seq,
                    type=EventType(row.type), timestamp=row.timestamp,
                    data=redacted_data, trace_id=trace_id,
                    correlation_id=correlation_id, sensitive=sensitive,
    ...
    def _row_to_event(r: RunEvent) -> AgentEvent:
        return AgentEvent(
            event_id=r.event_id, run_id=r.run_id, tenant_id=r.tenant_id,
            sequence=r.sequence, type=EventType(r.type), timestamp=_aware(r.timestamp),  # type: ignore[arg-type]
            data=dict(r.data_json or {}), trace_id=r.trace_id,
            correlation_id=r.correlation_id, sensitive=bool(r.sensitive),
    ...
    def create_approval(self, approval: Approval) -> Approval:
        with self.transaction() as s:
            ...
            s.add(ApprovalRow(
                approval_id=approval.approval_id, run_id=approval.run_id,
                tenant_id=approval.tenant_id, action_id=approval.action_id,
                requested_by=approval.requested_by, approver=approval.approver,
                decision=approval.decision.value, reason=approval.reason,
                requested_at=approval.requested_at, decided_at=approval.decided_at,
                expires_at=approval.expires_at, trace_id=approval.trace_id,
    ...
    def _row_to_approval(r: ApprovalRow) -> Approval:
        return Approval(
            approval_id=r.approval_id, run_id=r.run_id, tenant_id=r.tenant_id,
            action_id=r.action_id, requested_by=r.requested_by,
            approver=r.approver, decision=ApprovalDecision(r.decision),
            reason=r.reason, requested_at=_aware(r.requested_at),  # type: ignore[arg-type]
            decided_at=_aware(r.decided_at), expires_at=_aware(r.expires_at),
            trace_id=r.trace_id, correlation_id=r.correlation_id,
        )
    ...
    def record_tool_invocation(self, invocation: ToolInvocation,
        ...
        with self.transaction() as s:
            ...
            s.add(ToolInvocationRow(
                invocation_id=invocation.invocation_id, run_id=invocation.run_id,
                tenant_id=invocation.tenant_id, tool_name=invocation.tool_name,
                tool_version=invocation.tool_version, input_hash=in_hash,
                redacted_input_json=redacted_in, output_hash=out_hash,
                redacted_output_json=redacted_out, status=invocation.status.value,
                authorization_decision=(invocation.authorization_decision.value
                                        if invocation.authorization_decision else None),
                idempotency_key=invocation.idempotency_key, dry_run=invocation.dry_run,
                error_json=(dict(invocation.error) if invocation.error else None),
                trace_id=invocation.trace_id, correlation_id=invocation.correlation_id,
                started_at=invocation.started_at, completed_at=invocation.completed_at,
            ))
            s.flush()
            stored = invocation.model_copy(update={
                "input_hash": in_hash, "redacted_input": redacted_in,
    ...
    def _row_to_tool_invocation(r: ToolInvocationRow) -> ToolInvocation:
        ...
        return ToolInvocation(
            invocation_id=r.invocation_id, run_id=r.run_id, tenant_id=r.tenant_id,
            tool_name=r.tool_name, tool_version=r.tool_version,
            input_hash=r.input_hash, redacted_input=dict(r.redacted_input_json or {}),
            output_hash=r.output_hash, status=ToolInvocationStatus(r.status),
            authorization_decision=(AuthorizationDecision(r.authorization_decision)
                                    if r.authorization_decision else None),
            idempotency_key=r.idempotency_key, dry_run=bool(r.dry_run),
    ...
    def save_finding(self, *, finding_id: str, run_id: str, tenant_id: str,
                     title: str, summary: str, evidence_refs: list[str],
                     confidence: float, severity: str | None = None,
                     trace_id: str | None = None,
        ...
        with self.transaction() as s:
            ...
            s.add(FindingRow(
                finding_id=finding_id, run_id=run_id, tenant_id=tenant_id,
                title=title, summary=summary,
                evidence_refs_json=list(evidence_refs), confidence=confidence,
    ...
    def save_evidence_ref(self, *, ref_id: str, run_id: str, tenant_id: str,
                          kind: str, uri: str,
        ...
        with self.transaction() as s:
            ...
            if run is None or run.tenant_id != tenant_id:
                raise NotFoundError(f"run {run_id} not found")
            if s.execute(
                select(EvidenceRef).where(EvidenceRef.ref_id == ref_id)
            ).scalars().first() is not None:
                raise ConflictError(f"evidence ref {ref_id} exists")
            s.add(EvidenceRef(
                ref_id=ref_id, run_id=run_id, tenant_id=tenant_id,
                finding_id=finding_id, kind=kind, uri=uri,
                excerpt_hash=excerpt_hash, span_ref=span_ref,
            ))
            s.flush()
            return ref_id

    def save_agent_task(self, *, task_id: str, run_id: str, tenant_id: str,
                        skill_id: str, agent_name: str, objective: str,
                        status: str = "pending",
        ...
        with self.transaction() as s:
            ...
            s.add(AgentTask(
                ...
                skill_id=skill_id, agent_name=agent_name, objective=objective,
                status=status, input_hash=in_hash,
                redacted_input_json=redacted, deadline=deadline,
                created_at=_now(), updated_at=_now(),
            ))
            s.flush()
            return task_id

    def update_agent_task(self, *, task_id: str, tenant_id: str, status: str,
                          raw_output: dict[str, Any] | None = None,
    ...
    def record_model_call(self, *, call_id: str | None = None,
                          run_id: str, tenant_id: str, model: str,
        ...
        with self.transaction() as s:
            ...
            if run is None or run.tenant_id != tenant_id:
                raise NotFoundError(f"run {run_id} not found")
            s.add(ModelCall(
                call_id=cid, run_id=run_id, tenant_id=tenant_id, model=model,
                task_type=task_type, prompt_hash=prompt_hash,
    ...
    def write_audit(self, *, audit_id: str | None = None, run_id: str | None,
                    tenant_id: str, actor: str, action: str,
        ...
        with self.transaction() as s:
            if run_id is not None:
                ...
                if run is None or run.tenant_id != tenant_id:
                    raise NotFoundError(f"run {run_id} not found")
            s.add(AuditRecord(
                audit_id=aid, run_id=run_id, tenant_id=tenant_id, actor=actor,
                action=action, decision_json=redacted_decision, reason=reason,
                trace_id=trace_id, correlation_id=correlation_id,
                created_at=_now(),
            ))
            s.flush()
